...
const Wrapper = styled.div<{ $leftmargin: string }>`
  margin-top: 40px;
...
const MatrixCell = styled.td<{ $row?: number; $col?: number }>`
  ...
  background-color: ${(p) =>
    p.$col === 0 || p.$row === 0
      ...
      : p.$row && p.$row > 0 && p.$col !== undefined && p.$col > p.$row
  ...
  background-image: ${(p) =>
    p.$col === 0 || p.$row === 0
      ? 'revert'
      : p.$col !== undefined && p.$col > (p.$row ?? 0)
        ? 'var(--box-gradient)'
        : 'revert'};
  color: ${(p) =>
    p.$row && p.$row > 0 && p.$col !== undefined && p.$col > p.$row ? 'var(--number-box-font-color)' : 'white'};
`;
