...
function ShareDialog() {
  ...
  const sendEmail = async (shareId: string) => {
    const URL = AUTH_ENDPOINT + `email?id=${shareId}&recipient=${recipientEmail}`;
  ...
  const emailOption = isAuthenticated ? (
    <fieldset className='DialogFieldset'>
      ...
      <input className='DialogInput' id='email' placeholder='recipient@university.edu'
        value={recipientEmail} onChange={(event) => setRecipientEmail(event.target.value)} />
    </fieldset>
  ) : null;
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className='DialogContent'>
          ...
          <Dialog.Close asChild>
            <button className='DialogIconButton' aria-label='Close'><Cross2Icon /></button>
          </Dialog.Close>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
