...
try {
  ...
  // Amplify config may not be available in all environments
}

export interface UserQuery {
  MATRIX_SHAREID: { S: string };
  MATRIX_ULID: { S: string };
  MATRIX_DATA: { S: string };
  TITLE?: { S: string };
  CREATED_AT: { S: string };
  G_SEQUENCE: { S: string };
  F_SEQUENCE: { S: string };
...
export interface UserContextType {
  ...
  handleSignUp: (email: string, password: string, firstname: string, lastname: string) => Promise<void>;
  ...
  userDefaultHiddenSequences: string[];
  setUserSequences: React.Dispatch<React.SetStateAction<UserSequence[]>>;
  setUserDefaultHiddenSequences: React.Dispatch<React.SetStateAction<string[]>>;
...
function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const [userQueries, setUserQueries] = React.useState<UserQuery[]>([]);
  const [userSequences, setUserSequences] = React.useState<UserSequence[]>([]);
  ...
  function processUserSequences(sequences: any[]): UserSequence[] {
