...
function WindowControls() {
  const { sequenceLength, gSequence, fSequence, handleTruncateSequence, handleAugmentSequence } =
    React.useContext(DataContext);

  const numElements = Math.min(sequenceLength, fSequence.length, gSequence.length);
  const elements = gSequence.slice(0, numElements).map((num, index) => {
    const leftArrow = index === numElements - 1 ? (
      <TooltipWrapper message='Hide Last Term' side='top' sideOffset={5} arrowshiftX='0' arrowshiftY='0'>
        <Bubble onClick={handleTruncateSequence} $isleft='true'>
  ...
  const boxes = range(numAugmentBoxes).map((index) => {
    const rightArrow = index === 0 ? (
      <TooltipWrapper message='Reveal Next Term' side='top' sideOffset={5} arrowshiftX='0' arrowshiftY='0'>
        <Bubble onClick={handleAugmentSequence}><ChevronRightIcon width='18' height='18' /></Bubble>
      </TooltipWrapper>
    ) : '';
