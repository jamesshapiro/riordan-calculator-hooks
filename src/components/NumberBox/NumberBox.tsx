...
function NumberBox({ value, index, onSubmit, sequenceId, sequenceValue, isFirst, zIndex, disabled = false }: any) {
  const [digits, setDigits] = React.useState(value);
  const [isSelected, setIsSelected] = React.useState(false);
  const buttonRef = React.useRef<HTMLParagraphElement>(null);
  const divRef = React.useRef<HTMLDivElement>(null);
  const { targetBoxIndex, setTargetBoxIndex, handleSelectSequence, tabWasPressed, setTabWasPressed, handleTruncateSequence, handleSequenceChange } = React.useContext(DataContext);
  ...
  function handleKeyPress(value: string) {
    ...
    else if (value.startsWith('0') && value.length > 1) cleaned = value.slice(1);
  ...
  const inputNumberBox = (
    <form onSubmit={(event) => { event.preventDefault(); setIsSelected(false); onSubmit(index, digits); handleSelectSequence(sequenceId, 'custom'); }}>
      <StyledInput value={digits} $minfontsize={minfontsize} $fontSize={maxfontsizeRem} $maxfontsize={maxfontsize} onChange={(event) => handleKeyPress(event.target.value)} onBlur={handleBlur} onFocus={handleFocus} autoFocus />
    </form>
  );

  const boxContents = isSelected ? inputNumberBox : (
    <InnerElement $minfontsize={minfontsize} $fontSize={maxfontsizeRem} $maxfontsize={maxfontsize} ref={buttonRef}>{digits}</InnerElement>
  );
...
const Wrapper = styled.div<{ $disabled: boolean }>`
  ...
  border-radius: var(--number-box-border-radius); width: fit-content; min-width: var(--number-box-width); width: 100%; height: var(--number-box-height); margin: 1px; border: 1px solid var(--number-box-border-color); z-index: 1;
`;

const InnerContainer = styled.div<{ $disabled: boolean }>`
  display: flex; justify-content: center; align-items: center; font-family: 'Lato', sans-serif;
...
const StyledInput = styled.input<{ $minfontsize: string; $fontSize: string; $maxfontsize: string }>`
  ...
  font-size: clamp(${(p) => p.$minfontsize}, ${(p) => p.$fontSize}, ${(p) => p.$maxfontsize});
  height: 100%; width: max(var(--number-box-width), var(--number-box-width)); margin-left: 2px; text-align: center;
  line-height: normal; border: 1px solid var(--number-box-hover-font-color); background-color: white; box-sizing: border-box;
`;
