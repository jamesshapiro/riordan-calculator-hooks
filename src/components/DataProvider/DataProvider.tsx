...
function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchOeisSequence = React.useCallback(async (sequenceId: string, authToken: string) => {
  ...
  function getTwoBellSequence(gSeq: number[]) {
    ...
    for (let i = 0; i < gSquared.length; i++) {
      ...
      gSquared[i] = unreducedProduct.reduce((a, b) => a + b, 0);
    }
    return [0, ...gSquared];
  }
  ...
  const setOeisToF = React.useCallback(
    ...
    [mode, metaMode, gSequence, fSequence]
  );
  ...
  React.useEffect(() => {
    async function fetchMatrix(
      ...
      fSeq: number[],
      currentMode: string,
      currentMetaMode: string
    ) {
      const payload = {
        g: gSeq.slice(0, seqLength),
        f: fSeq.slice(0, seqLength),
        gSequenceId: currentGSelection,
        fSequenceId: currentFSelection,
        sequenceLength: seqLength,
        mode: currentMode,
        metaMode: currentMetaMode,
      };
